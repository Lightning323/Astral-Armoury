...
public class PotionsRegistry {
    ...
    public static void setup() {
        ...
        basicBrewing(FREEZE_POTION.get(), slowPotion.copy(), AstralItems.SNOWFLAKE.get()); //freeze

        final ItemStack freeze_potion = PotionUtils.setPotion(new ItemStack(Items.POTION), FREEZE_POTION.get());
        basicBrewing(FREEZE_POTION_2.get(), freeze_potion.copy(), AstralItems.GOLD_SNOWFLAKE.get());//freeze 2


        //Reach distance
        if (REACH_POTION != null) {
            basicBrewing(REACH_POTION.get(), awkwardPotion.copy(), AstralItems.GEM_AMETHYST.get());
        }
        ...
        if (ATTACK_RANGE_POTION != null) {
            ...
            basicBrewing(ATTACK_RANGE_POTION.get(), reach_potion.copy(), AstralItems.GEM_PRISMARINE.get());
        }
    }
